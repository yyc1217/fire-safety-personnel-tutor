...
`notes` 模式下 exam-tutor 產出的弱點筆記，**日後會由使用者提供回來供弱點複習選題**，格式須固定才能對回標籤索引（每考點固定 `tag：` 行對齊 `tags_index.json` 維度、`題目參照：` 行沿用 `q_id` 格式，讀回以 `tag：` 行為準）。**版面、規則與範例見 `reference/輸出格式/弱點筆記格式.md`**（格式唯一真相）；存 `notes/<YYYY-MM-DD>_<主題>弱點筆記.md`（命名見下節）。
