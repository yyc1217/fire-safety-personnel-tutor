...
```json
{
  ...
  "coverage": {
    ...
    "by_article:設置標準第31條": {"done": ["效能值核算", "場所面積門檻100/200"]},
...
- **`coverage`＝內容覆蓋度紀錄（掌握度的資料核心）**：記錄使用者對各**設備／法條／火災學主題**之「相關內容」已展現掌握的部分，供 `/掌握度` 計算覆蓋度，並供「還有哪些沒讀到」與主動提議下一個單元使用。每筆 `{"done": [已掌握內容點], "next"?: <建議下一個內容點>}`：
  - **設備**（key＝**設備名**，維持既有格式，向後相容）：`done`＝已覆蓋之**條文**，分母＝`reference/設備條文索引.md` 該設備全部條文＋延伸知識考點。
  - **法條**（key＝`by_article:<條號短名>`）：`done`＝已展現掌握之**該條內容要點**（以該條之項／款或關鍵內容為單位；批改申論時以擬答是否涵蓋該條內容判定），分母＝該條之內容要點數（取自 `statutes/` 該條全文結構；無細分時整條計 1 點）。
  - **火災學主題**（key＝`by_topic:<主題>`）：`done`＝已展現掌握之**知識點**，分母＝`reference/火災學主題知識點索引.md` 該主題知識點總數。
