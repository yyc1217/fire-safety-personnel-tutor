...
`notes` 模式下 exam-tutor 產出的弱點筆記，**日後會由使用者提供回來供弱點複習選題**，格式須固定才能對回標籤索引。存 `notes/<YYYY-MM-DD>_<主題>弱點筆記.md`（命名見下節），逐區塊比照：
...
### {考點中文名}（錯 {n} 次）
- tag：`by_equipment:{設備}`、`by_article:{法規條號}`
- 題目參照：`{等別/年/科目代號#題號}`、…
- 錯誤型態：{一句描述錯在哪}
...
- 每個弱點考點一個 `###` 分節，**標題括號記累計錯誤次數**（對應 `weak_tally` 語意）。
- `tag：` 行的 key **對齊 `corpus/tags_index.json` 維度**（`by_equipment:`／`by_article:`／`by_system:`）；讀回選題時**以 `tag：` 行為準**，其餘欄位供人閱讀。
- `題目參照：` 沿用 `q_id` 格式（`等別/年/科目代號#題號`），供優先重考曾答錯之原題。
- 公式一律 LaTeX。同主題多次練習：使用者可自行併檔或累積多檔，讀回時以各檔 `###` 標題彙總次數。

## 讀書計畫檔格式（`plans/讀書計畫_<等別><民國年>.md`）

study-planner 產出的計畫檔**跨對話讀回解析**（勾銷、重排、進度報告），格式須固定。兩種形式共用同一檔首：
...
**（a）順序清單**——`##` 依法規大類分節（標該等別題數），單元行固定三段式「單元（條文）｜建議行動｜對照表」：
