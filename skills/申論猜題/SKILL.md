...
**必產出**（依 exam-tutor「申論猜題＋擬答模式」規範）：預測題目（仿歷屆題型與配分，依 `level`）＋答題架構（破題→法源→分項論述→結論）＋擬答範例——每一論點附法源（法規名＋條項款＋版本日期，無法源之推論標注「非法源」）、**逐步推導**（要件→法源→涵攝→小結），計算題每步公式標明法源、代值附單位不跳步，直接寫結論不給高分。使用者同意時存 `<data_dir>/forecasts/申論猜題擬答_<等別><民國年>_<科目>.md`。
