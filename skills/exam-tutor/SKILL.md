...
| **消防圖說圖例** | `statutes/2_02_消防機關辦理建築物消防安全設備審查及查驗作業基準/原始檔案/附件三：消防圖說圖示範例.md`（17 類 284 個圖例；圖檔在同層 `附件三_圖例/`，機器索引 `附件三_圖例_index.json` 可用 jq 抽題） | **識圖題**出題來源（不在命題大綱明列，但為考題來源）：看圖答名、依名選圖（以同類別相似圖例作干擾選項）、加註規則（藥劑、等級、W 數、住警器 R） |
...
**`statutes/2_01_各類場所消防安全設備設置標準/2_01_各類場所消防安全設備設置標準.md` 特別注意**：該檔 4,239 行，**超過 Read 工具預設 2,000 行上限**，整檔 Read 會被截斷且靜默取到不完整內容（可能漏掉後半部條文而誤判「查無此條」）。取該檔條文**必須**走「定位行號 → Read 帶 offset」流程（用哪種方式定位不限）。其餘法規 md 均在上限內，但仍以精準取用為原則。
...
0. **先正規化設備名**（不額外花呼叫，就在下一步那次 Grep 裡完成）。使用者給的常是口語詞、條文標題或縮寫（「緊急照明燈」「撒水頭」「探測器」「住警器」）。以使用者字串 Grep 索引後，**一律以命中列的「設備分類」欄作為後續 `tags_index`／`coverage` 查詢的鍵**，**不得**拿使用者原字串去 `jq`——查不到會回 `null`，整個設備的題目一題都取不到。
   ...
   - **兩個必須併查的例外**：
     ...
     - `消防幫浦/加壓送水裝置` **是獨立的出題單元**（151 題，水系統高頻考點），課綱列見設備條文索引「水設備」節：主體為 `statutes/2_07_消防幫浦加壓送水裝置等及配管摩擦損失計算基準/2_07_消防幫浦加壓送水裝置等及配管摩擦損失計算基準.md` 之五節（通則／幫浦／電動機／附屬裝置等／配管摩擦損失計算），另含各水系統的加壓送水裝置條文（室內消防栓 §37、室外 §42、自動撒水 §58、水霧 §65、泡沫 §77／§221）與設置標準 §234（授權幫浦基準）。
       **那六條是共用條文，同時列在該水系統名下**（比照 §18 同時掛在二氧化碳／泡沫／乾粉／鹵化烴／水霧之下的既有作法）：出到「室內消防栓」單元時照該設備的順序走，出到「消防幫浦」單元時則以幫浦基準為主軸、共用條文作為各系統的具體適用例。**覆蓋度分別計算，不互相抵銷**。
