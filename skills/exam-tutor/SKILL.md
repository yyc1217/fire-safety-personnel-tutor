...
0. **先正規化設備名**（不額外花呼叫，就在下一步那次 Grep 裡完成）。使用者給的常是口語詞、條文標題或縮寫（「緊急照明燈」「撒水頭」「探測器」「住警器」）。以使用者字串 Grep 索引後，**一律以命中列的「設備分類」欄作為後續 `tags_index`／`coverage` 查詢的鍵**，**不得**拿使用者原字串去 `jq`——查不到會回 `null`，整個設備的題目一題都取不到。
   ...
   - **兩個必須併查的例外**：
     ...
     - `消防幫浦/加壓送水裝置` **是獨立的出題單元**（151 題，水系統高頻考點），課綱列見設備條文索引「水設備」節：主體為 `statutes/2_07_消防幫浦加壓送水裝置等及配管摩擦損失計算基準.md` 之五節（通則／幫浦／電動機／附屬裝置等／配管摩擦損失計算），另含各水系統的加壓送水裝置條文（室內消防栓 §37、室外 §42、自動撒水 §58、水霧 §65、泡沫 §77／§221）與設置標準 §234（授權幫浦基準）。
