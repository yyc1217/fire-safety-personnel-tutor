...
本輪完成幾題，並給「接續作答／丟掉重開／先看掌握度」三選項，接續時把 `pending.items[].question_text`
**原樣貼回**；逾 14 天預設重開；`session_id` 不同且未滿 30 分鐘視為另一視窗正在練習，先問再動；
等別不符預設丟棄。**沒有 `pending` 就直接開始，不要為此多問一句。**
...
5. **記錄內容覆蓋度（`weakness_tracking = "auto"` 時）**：**每題批改完就地寫入，不得累積到一輪結束**（見 `user-config-spec.md`「寫入時機與併發」）——`attempts.jsonl` append 一行、更新 `weak_tally`、把該題自 `pending.items` 移除並將成績記入 `pending.round.scored`；同時判定使用者本題擬答**已展現掌握之內容點**，累積寫入 `progress.json` 之 `coverage`（schema 見 `user-config-spec.md`），供 `/掌握度` 計算覆蓋度：
...
| `pending` 之 `session_id` 非本對話且未滿 30 分鐘 | 另一視窗可能正在練習：**不覆寫**，先問使用者要接手還是另開一輪 |
| `progress.json` 損毀或解析失敗 | 保留原檔（不清空重建），改由 `attempts.jsonl` 重算 `weak_tally`／`coverage`；兩者皆不可讀時如實告知並以無紀錄狀態繼續上課 |
