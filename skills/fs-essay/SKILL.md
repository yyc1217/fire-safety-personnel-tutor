---
name: fs-essay
description: 【申論猜題】申論題猜題＋擬答：依猜題範圍或指定科目預測申論題目，附答題架構與擬答（每一論點附法源）
...
**本指令不是 `/fs-forecast`**：不要改跑 exam-trend-forecast／`/fs-forecast`，也**不得只輸出「猜題範圍」報告就停下、更不得叫使用者再呼叫一次 `/fs-essay`**。猜題範圍只是選題依據，本指令的交付物是「預測題目與擬答」。
