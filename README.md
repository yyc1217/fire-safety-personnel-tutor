...
| 指令 | 功能 | 範例 |
|------|------|------|
| `/fs-quiz [範圍?]` | 快速抽考一輪 3~5 題 | `/fs-quiz 警報系統` |
| `/fs-mock [科目?]` | 套用最新年度原卷版面（首尾標註【模擬考】）產出整卷仿真模擬試卷＋標準答案。**在獨立子代理中執行**，命題過程不佔主對話脈絡 | `/fs-mock 消防法規` |
| `/fs-weak [範圍?]` | 依作答紀錄優先重考常錯考點 | `/fs-weak` |
| `/fs-mastery [範圍?]` | 把本機作答紀錄畫成各主題／設備／法條的內容覆蓋度文字條地圖（掌握度＝已展現掌握之內容點 ÷ 相關內容總點數），一眼看出哪裡強、哪裡還沒碰 | `/fs-mastery 水系統` |
| `/fs-compare [主題?]` | 跨法規對照表；留空列出內建清單 | `/fs-compare 各設備緊急電源` |
| `/fs-forecast [範圍?]` | 產出猜題範圍（考點＋依據＋週期型態）：先秒回本地統計結果，詢問後才可選上網補近期動態 | `/fs-forecast` |
| `/fs-essay [科目?]` | 申論猜題＋答題架構＋擬答（每一論點附法源） | `/fs-essay 消防法規` |
| `/fs-cram [範圍?]` | 猜題範圍＋平時弱點濃縮成必背文件 | `/fs-cram` |
