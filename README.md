...
**出題練習**——「考我滅火器」：依設備條文索引連續出題（§14 應設場所 → §31 設置規定 → 公危 §223～§228 → 檢修基準 → 延伸知識），出題後停下等作答，批改附條文全文與法源出處。未指定設備時主動提議（讀書計畫 → 弱點 → 頻率）。

**系統複習**——「帶我看水系統的規定」：整批講解該系統條文、設備、檢修要領與常考數字，講解為主、測驗為輔。

**考前猜題**——`/猜題`：產出如「【時事預測｜信心：中】去年○○瓦斯氣爆 → 優先準備瓦斯漏氣火警自動警報設備（設置標準 §21、§140～§145-1）」的猜題範圍；想練題接 `/申論猜題`、想背重點接 `/懶人包`。

**申論擬答**——`/申論猜題 消防法規`：預測題目＋答題架構＋擬答，每一論點附法源（如「依《消防法》§9Ⅰ①…（113-11-29 修正版）」），無法源之推論標注「非法源」。

**考前衝刺**——`/懶人包`：猜題範圍＋你的常錯考點（標注「⚠ 你的弱點」優先排序）濃縮成必背文件；`/弱點複習`：重考錯過的題目直到克服。

**規劃進度**——`/讀書計畫 115-06-14 每週10小時`：依命題頻率產逐週排程，考前 1~2 週自動預留總複習與猜題；之後出題會優先提議計畫中下一個未完成單元。

**查卷**——「給我 113 年設備師消防法規的原卷」：提供原卷 PDF 與 md 全文；「列出考過緩降機的題目」：依標籤查題。
